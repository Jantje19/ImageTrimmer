...
namespace ImageTrimmer
{
				...
				{
								/* Thanks: http://pietschsoft.com/post/2009/08/17/CSharp-IProgressDialog-Show-Native-Progress-Dialog-from-dotNet-in-Windows */
								...
								public void Show(string title, string message, string cancelMessage, params PROGDLG[] flags)
								{
												if (pd == null)
												{
																pd = (Win32IProgressDialog)new Win32ProgressDialog();

																pd.SetTitle((title == null) ? "Progress" : title);
																pd.SetCancelMsg(cancelMessage, null);
																
																if (message != null)
																				pd.SetLine(1, message, false, IntPtr.Zero);
								...
								public uint SetProgess(uint progress)
								{
												pd.SetProgress(progress, 100);
												return progress;
								}
